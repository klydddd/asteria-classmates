...
from bosesph.ingestion import METADATA_COLUMNS
...
def _compute_stats(
    ...
    for split in (DatasetSplit.TRAIN, DatasetSplit.VALIDATION, DatasetSplit.TEST):
        ...
        split_data[split.value] = SplitDistribution(
            ...
            duration_seconds=sum(
                float(row["duration_seconds"]) for row in split_rows
            ),
        )
    ...
    return DatasetStats(
        ...
        average_clip_seconds=round(total_duration / total_clips, 3)
        if total_clips
        else 0.0,
...
def _generate_dataset_card(stats: DatasetStats) -> str:
    ...
    lines = textwrap.dedent(f"""\
        ...
        |---|---:|---:|---|
        | Train | {stats.splits.get("train", SplitDistribution(clips=0, speakers=0, duration_seconds=0)).clips} | {stats.splits.get("train", SplitDistribution(clips=0, speakers=0, duration_seconds=0)).speakers} | {_format_duration(stats.splits.get("train", SplitDistribution(clips=0, speakers=0, duration_seconds=0)).duration_seconds)} |
        | Validation | {stats.splits.get("validation", SplitDistribution(clips=0, speakers=0, duration_seconds=0)).clips} | {stats.splits.get("validation", SplitDistribution(clips=0, speakers=0, duration_seconds=0)).speakers} | {_format_duration(stats.splits.get("validation", SplitDistribution(clips=0, speakers=0, duration_seconds=0)).duration_seconds)} |
        | Test | {stats.splits.get("test", SplitDistribution(clips=0, speakers=0, duration_seconds=0)).clips} | {stats.splits.get("test", SplitDistribution(clips=0, speakers=0, duration_seconds=0)).speakers} | {_format_duration(stats.splits.get("test", SplitDistribution(clips=0, speakers=0, duration_seconds=0)).duration_seconds)} |
        ...
        *Generated by BosesPH Toolkit on {datetime.now(timezone.utc).strftime("%Y-%m-%d")}*
    """)
    return lines
...
def build_dataset(
    ...
    *,
    ...
    if not approved_rows:
        raise DatasetBuildError(
            "no approved clips found; run review before building"
        )
